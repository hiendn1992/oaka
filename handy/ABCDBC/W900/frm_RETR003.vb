...
Public Class frm_RETR003 : Inherits jp.co.ait.common.forms.ABCDBCForm
...
#Region " MENTHODS "
    ''' <summary>
    ''' frm_Retrieve003_Load
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frm_Retrieve003_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try

            WriteEventStartLog("frm_Retrieve003_Load")
            ...
            lbl_imp_count.Text = BCLoginManager.RetrieveW900Count.ToString()
        ...
        Catch ex As Exception
            ...
            End If
        Finally
            WriteEventEndLog("frm_Retrieve003_Load")
        End Try
    End Sub
        ...
        Try
            ...
            If barcodeObj.StrCodedata.Length <> 14 Then
                ...
                If Not Me.msgListener Is Nothing Then
                    Me.msgListener.Dispose()
                End If
                ShowMessageForm("ERR121", "Retrieve  W900  Error")
            ...
            Dim ds As DataSet = webService.CheckRetrieveBarcodeExistSelectedRack(barcodeObj.StrCodedata, rackCd, BCLoginManager.GetLoginInfo.UserID)
            ...
            If result = 0 Then ' check success
                ...
                ' import barcode
                webService.RetrieveBarcodeIntoQC(barcodeObj.StrCodedata, BCLoginManager.GetLoginInfo.UserID, itemCd, rackCd)
                BCLoginManager.RetrieveW900Count = BCLoginManager.RetrieveW900Count + 1
                Dim msg As String = String.Format("Retrieve {0} Sucessfully !", Environment.NewLine)
                retr005 = New frm_RETR005(msg, rackCd)
                retr005.Show()
                '>>
                'retr004 = New frm_RETR004(barcodeObj.StrCodedata, itemCd, itemName, rackCd)
                'retr004.Show()
                Me.Close()
            ...
            ElseIf result = 2 Then
                ...
                End If
                'barcodeObj.ScanDisable()
                ShowMessageForm("ERR122", "Retrieve  W900  Error")
                Me.Focus()
                msgListener = New MessageListener(Me)
            ElseIf result = 3 Then
                ...
                End If
                'barcodeObj.ScanDisable()
                ShowMessageForm("ERR143", "Retrieve  W900  Error", , New String() {rackCd})
                Me.Focus()
                msgListener = New MessageListener(Me)
            End If
        Catch ex As Exception
    ...
    ''' <summary>
    ''' frm_Retrieve003_Closing
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frm_Retrieve003_Closing(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
        Try
            WriteEventStartLog("frm_Retrieve003_Closing")
        ...
        Catch ex As Exception
            ...
            End If
        Finally
            WriteEventEndLog("frm_Retrieve003_Closing")
        End Try
    End Sub
