...
Public Class frm_REJ001 : Inherits jp.co.ait.common.forms.ABCDBCForm

#Region " VARIABLE / CONSTANT "

    Private frm_Rej2 As frm_REJ002
    Private frm_Rej3 As frm_REJ003
...
#Region " MENTHODS "
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frm_Reject001_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try

            WriteEventStartLog("frm_Reject001_Load")
            ...
            lbl_imp_count.Text = BCLoginManager.RejectW900Count.ToString()
        ...
        Catch ex As Exception
            ...
            End If
        Finally
            WriteEventEndLog("frm_Reject001_Load")
        End Try
    End Sub
        ...
        Try
            ...
            If barcodeObj.StrCodedata.Length <> 14 Then
                ...
                If Not Me.msgListener Is Nothing Then
                    Me.msgListener.Dispose()
                End If
                ShowMessageForm("ERR121", "Reject  W900 Error")
                Me.Focus()
            ...
            Dim ds As DataSet = webService.CheckRejectBarcodeExistProcess(barcodeObj.StrCodedata, BCLoginManager.GetLoginInfo.UserID)
            ...
            If result = 0 Then ' check success
                ...
                ' import barcode
                webService.RejectBarcodeIntoW9902(barcodeObj.StrCodedata, BCLoginManager.GetLoginInfo.UserID, itemCd)
                ...
                frm_Rej3 = New frm_REJ003(msg)
                frm_Rej3.Show()
                '>>
                'frm_Rej2 = New frm_REJ002(barcodeObj.StrCodedata, itemCd, itemName)
                'frm_Rej2.Show()
                Me.Close()
            ...
            ElseIf result = 2 Then
                ...
                If Not Me.msgListener Is Nothing Then
                    Me.msgListener.Dispose()
                End If
                ShowMessageForm("ERR122", "Reject  W900 Error")
                Me.Focus()
                msgListener = New MessageListener(Me)
            ElseIf result = 3 Then
                ...
                End If
                'barcodeObj.ScanDisable()
                ShowMessageForm("ERR124", "Reject  W900 Error")
                Me.Focus()
                msgListener = New MessageListener(Me)
            End If
        Catch ex As Exception
    ...
    ''' <summary>
    ''' frm_Reject001_Closing
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frm_Reject001_Closing(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
        Try
            WriteEventStartLog("frm_Reject001_Closing")
        ...
        Catch ex As Exception
            ...
            End If
        Finally
            WriteEventEndLog("frm_Reject001_Closing")
        End Try
    End Sub
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frm_Reject001_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
